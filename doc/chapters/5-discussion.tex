...
The fingerprint clusters visualized in Figure \ref{figure:clusters} suggest that histogram method outperforms the fingerprint method. It is able to find 50\% more clusters in the corpus with a coverage approximately 95\% (vs. the 67\% produced by the fingerprint method). Furthermore, the clusters are of better ``quality'': there are several groups of 2-4 sibling fingerprints -- clusters that have similar size and color.
