...
The most significant pitfall of the fingerprint method is the static nature of its analysis and matching parameters. Figure \ref{figure:fingerprint-method-pitfalls} illustrates two example problem cases.
...
\begin{figure}[h!]
  ...
  \caption{A fixed persistance threshold (red line) provided inaccurate results when SNR was low (left image). Moreover, a fixed persistance threhold was unable to account for slight variations in the hue distribution -- caused by e.g. residual afterglow from a previous capture -- leading to inconsistent results (right image). The color bars on the x-axis depict the relative amounts of different hues in the histogram.}
  \label{figure:fingerprint-method-pitfalls}
\end{figure}

Based on the results and the findings presented above the following can be concluded:

\begin{itemize}
\item - histogram outperforms fingerprint method
\item - lumia outperforms s4
\item - long intervals provide better results
\item - taggant preparation process has a great impact
\item - fewer frames could suffice(?)
\item - the static nature of the fingerprint method parameters hinder its accuracy
\end{itemize}
