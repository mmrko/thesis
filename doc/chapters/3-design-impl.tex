...
\begin{itemize}[label=]
    ...
    \item $n$: sample of fingerprint
    \item $i$: peak in sample
    \item $w_x$: weight for property $x$
    \item $\Delta{x_i}$: delta between peak $i$ and its pair for property $x$
...
In order to compute $P_x$ the peaks of the subject and candidate fingerprints have to be paired with each other. The pairing of the peaks boils down to an assignment problem where each peak in sample $A_n$ of fingerprint $A$ needs to be paired with a peak in sample $B_n$ of fingerprint $B$ with minimal cost (change in hue). One suitable algorithm for solving the assignment problem for small datasets is the Hungarian algorithm \cite{hungarian_algorithm}. The algorithm takes an $n \times m$ \emph{cost matrix} as input and outputs a $min (n,\ m)$ number of $(i, j)$ coordinates (pairs) that represent the minimum cost.

The cost matrix $C$ is formed by computing the peak hue deltas ($\Delta{h_i}$) between samples $A_n$ and $B_n$. Each matrix cell $a_{ij}$ represents the hue delta between the $i$th peak of $B_n$ and $j$th peak of $A_n$. Thus, the dimensions of the matrix are defined by the number of peaks in the samples. The cost matrix and the pairings made by the algorithm are illustrated in Figure \ref{figure:hungarian-algorithm}.
...
The Hungarian algorithm does not, however, provide a solution for the \emph{unbalanced assignment problem}: non-square cost matrices ($n \neq m$) are always left with unpaired rows/columns as the algorithm only returns $min (n,\ m)$ number of pairs. This becomes an issue in cases where samples $A_n$ and $B_n$ have a different number of peaks. To overcome this limitation the algorithm is extended by a custom implementation, which finds the lowest-cost pair for any unpaired rows/columns (peaks). The logic of the extended functionality supporting non-square cost matrices is illustrated in Figure \ref{figure:hungarian-algorithm-extended}.

% This of course means that some peaks will in the end have more than one pair, which however is not an issue for the use case at hand
...
The code written for computing the peak pairs uses an open-source JavaScript implementation of the Hungarian algorithm underneath \cite{munkres-js}. Peakless samples were (for which the algorithm would return no pairs) were given a fixed penalty of (peak count to account for peakless fingerprints)
