...
The \emph{Past traces} view displays a history of successfully matched products and allows the user to navigate further to view the relevant product information (e.g. product description and link to an online marketplace). The \emph{Settings} sidebar mainly consists of parameters for configuring the underlying algorithms and the smartphone's camera. These settings were only exposed for research and development purposes and would not be displayed to the end-user in the actual application. The main interaction endpoint for the user is the red, circle capture button on the landing page, which once tapped on, initiates the fingerprint capture process. The fingerprint is analyzed by the application to determine whether or not the given taggant matches a record in the database. This process is discussed in detail in chapters \ref{chapter:fingerprint-pipeline} and \ref{chapter:storage-security}.

To interact with the native Android and Window Phone camera APIs a custom Cordova plugin was implemented to interact. Existing Cordova camera plugins could not be utilized...
configurable parameters

As a part of the capture process the application utilizes an external camera module. The implementation of the camera module is dicussed in the next chapter.

\subsection{Camera Module}
\label{chapter:camera-module}

The camera application is accompanied by an external camera module that consists of a microcontroller, a light source, and a photoresistor. The purpose of the camera module is to provide a fixed source of light for photoexcitation. The camera module  interacts with the camera application by light signals. The embedded microcontroller uses a Light Dependent Resistor (LDR) to detect light emitted by the smartphone. When the smartphone's camera flash blinks the microcontroller triggers the light source, which in turn emits the appropriate wavelength of light to activate the taggant (excite the luminophore). The architecture of the camera module is illustrated in Figure \ref{figure:camera_module}.



\begin{figure}[h]
\centering \includegraphics[width=13cm]{images/design_implementation/camera_module.pdf}
\caption{The camera module consists of an external light source, photoresistor and a microcontroller encased in cardboard. \label{figure:camera_module}}
\end{figure}
